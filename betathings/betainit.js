...
async function initBot(bot, message){
	...
	if (bot.dontberetarded == undefined){
		...
		console.log = function(msg){
			...
			if (bot.loggedIn){
				...
				logchannel = logguild.channels.cache.get('687149197199147038');
	...
	if (message != null){
		...
		const restarted = JSON.parse(fs.readFileSync('./betathings/betaconfigs/restart.json', 'utf8'));
		if (restarted.restart == true){
			...
			fs.writeFileSync('./betaconfigs/restart.json', json, 'utf8');
		}
    }
